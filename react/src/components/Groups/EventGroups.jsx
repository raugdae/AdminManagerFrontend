...
function EventGroups() {
  ...
  const handleGroupAdd = async () => {
    ...
    const body = { groupname: addGroupName, parentid: addGroupParent };


    await addEventGroup(localStorage.getItem('eventID'),body)
    fetchData();
    setIsModalOpen(false);
  };
  ...
  return (
    <div className="flex flex-col m-4">
      <div className="flex justify-center ">
        <button
          className="border-lime-800 border-4 bg-emerald-700 hover:bg-green-300 "
      ...
      {dataset.map((item, index) => (
        <div
          key={index}
          className="flex flex-row m-2 justify-around border-b items-center"
        >
          {Object.values(item).map((value, i) => {
            if (
              Object.keys(item)[i] !== "id" &&
              Object.keys(item)[i] !== "fk_parentgroupid"
            )
              return (
        ...
        {modalType === "Edit" && (
          <div className="flex flex-col bg-gradient-to-b from-emerald-500 to-emerald-900">
            <div className="relative flex justify-between items-center">
              ...
              <div className="font-extrabold ml-4">
                <h2>
                  Modifier le groupe <b>{selectedItem.groupe} </b>
                </h2>{" "}
              </div>
              <button
                onClick={closeModal}
                className="text-gray-500 hover:text-red-700 transition-colors m-4"
              >
                <X />
              </button>
            </div>
            <div className="space-y-4 flex flex-col justify-center">
              {Object.keys(selectedItem).map((key) => {
                if (key === "groupe") {
                  return (
                    <div key={key} className="relative flex flex-col m-4">
                ...
                if (key === "parent") {
                  return (
                    <div className="m-4 relative flex flex-col">
                      ...
                      <select
                        className="bg-zinc-200 border-2 px-3 py-2"
                      ...
                      >
                        <option key="x" value={null}>---</option>
                        {dataset.map((item, index) => {
            ...
            <div className="flex justify-center p-4 w-full h-full bg-emerald-200">
                <div className="flex flex-col relative gap-2">
                  ...
                  <input
                    type="text"
                    name="groupName"
                    className="bg-zinc-100 px-3 py-2"
                    onChange={(e) => {
                  ...
                  <select
                        ...
                        onChange={(e) => {
                      setAddGroupParent(e.target.value);
                    }}
                      >
                        <option key="x" value={null}>---</option>
                        {dataset.map((item, index) => {
